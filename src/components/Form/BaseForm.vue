<template>
  <div class="form">
    <div v-if="formItem && formItem.length > 0 && dialogVisible">
      <el-form ref="commonForm" size="small" :rules="rules" :model="form" :inline="true" label-position="left"
          ...
          <el-col v-for="(item, index) in formItem" :key="index" :span="item.width">
            <el-form-item :key="index" class="table-header-item" :label="item.label" :prop="item.value">
              ...
              <el-select v-if="item.type === 'select'" v-model="form[item.value]" clearable
                ...
                :multiple="item.multiple ? item.multiple : false" :disabled="item.disabled"
                :placeholder="`请选择${item.placeholder || ''}`" @change="emit('change', form, item.value)">
                <el-option v-for="(option, i) in item.children" :key="i" :value="option.value" :label="option.label" />
              </el-select>
              <!--文本框-->
              <el-input v-if="item.type === 'text'" :id="item.value" v-model.trim="form[item.value]" clearable
                :placeholder="item.disabled === true ? '' : `请输入${item.placeholder || ''}`" :maxlength="item.maxlength"
                :oninput="emit('oninput', item)" :disabled="item.disabled" @change="emit('change', form, item.value)" />
              <!-- 密码框 -->
              <el-input v-if="item.type === 'password'" v-model="form[item.value]" type="password"
                :placeholder="item.placeholder ? item.placeholder : '请输入'" show-password :disabled="item.disabled" />
              <!-- 内置弹窗 -->
              <div v-if="item.type === 'inputBtn'" class="input-button">
                <el-input :id="item.value" v-model.trim="form[item.value]" class="input-button-left"
                  :placeholder="item.disabled === true ? '' : `请选择${item.placeholder || ''}`"
                  :oninput="emit('oninput', item)" :disabled="item.disabled" />
                <el-button class="input-button-right" :disabled="item.disabled" @click="emit('focus', item, form)" />
              </div>

              <!--数字-->
              <el-input v-if="item.type === 'number'" :id="item.value" v-model.number="form[item.value]" clearable
                :placeholder="item.disabled === true ? '' : `请输入${item.placeholder || ''}`" :maxlength="item.maxlength"
                :oninput="emit('oninput', item)" :disabled="item.disabled" @change="emit('change', form, item.value)" />
              <!--文本域-->
              <el-input v-if="item.type === 'textarea'" v-model="form[item.value]" clearable
                :rows="item.rows ? item.rows : 3" type="textarea" :placeholder="`${item.placeholder || ''}`"
                :maxlength="item.maxlength" show-word-limit resize="none" />
              <!-- 单选 -->
              <el-radio-group v-if="item.type === 'radio'" v-model="form[item.value]" :disabled="item.disabled"
              ...
              <!-- 联级选择器 -->
              <el-cascader v-if="item.type === 'cascader'" v-model="form[item.value]" :options="item.children"
                :props="item.props" :show-all-levels="false" :disabled="item.disabled" @change="handleChange" />
              <!-- 日期 -->
              <el-date-picker v-if="item.type === 'date'" v-model="form[item.value]" type="date"
                :disabled="item.disabled" placeholder="选择日期" />
              <!-- 上传 -->
              <el-upload v-if="item.type === 'upload'" ref="upload" class="upload-file" :action="''"
                :accept="item.accept" :http-request="uploadFile" :before-upload="beforeUpload" :on-remove="onRemove"
                :limit="item.limit" :multiple="item.multiple" :on-exceed="handleExceed" :file-list="form.fileList">
              ...
              <!-- 输入框样式点击弹窗 -->
              <div v-if="item.type === 'click'" class="clickDialog" :class="{ textColor: item.leaderName }"
                @click="item.click">
