<template>
  <el-table v-bind="$attrs" border ref="tableDom" v-loading="listLoading" :data="tableData" stripe height="100%"
    @selection-change="handleSelectionChange" @select-all="onSelectAll">
    <el-table-column v-if="selection" type="selection" width="40" />
    <!-- 序号 -->
    <el-table-column width="60" align="center" label="序号" :show-overflow-tooltip="true">
      <template v-slot="scope">
        <span>{{ (formselect.page - 1) * formselect.limit + scope.$index + 1 }}</span>
      </template>
    </el-table-column>
    <el-table-column v-for="item in tableHead" :key="item.prop" :prop="item.prop" :label="item.label"
      :width="item.width + 'px'" show-overflow-tooltip />
    <el-table-column v-if="operation.length" label="操作" :width="operation.length && operation.length * 23 + 55">
      ...
      <template #default="scope">
        <div class="handle">
          <img v-if="operation.indexOf('view') > -1" src="@/assets/img/table/check.png" class="view" title="查看" alt="查看"
            @click="emit('handleView', scope.row)">
          <img v-if="operation.indexOf('edit') > -1" src="@/assets/img/table/edit.png" class="edit" title="编辑" alt="编辑"
            @click="emit('handleEdit', scope.row)">
          <img v-if="operation.indexOf('del') > -1" src="@/assets/img/table/delete.png" class="del" title="删除" alt="删除"
            @click="emit('handleDelete', scope.row)">
        </div>
      </template>
    </el-table-column>
  </el-table>
  <CustomPagination :total="total" @pagination="pagination" />
</template>
<script setup>
